...
//       <div class="product-card" style="border:5px solid black;" data-aos="fade-up" data-aos-delay="300" tabindex="0">
...
function renderProducts() {
  ...
  const productsPerRow = 4; // Adjust this based on your layout
  ...
  productKeys.forEach((key, index) => {
    ...
    // Determine AOS delay for rows
    const rowIndex = Math.floor(index / productsPerRow);
    const aosDelay = rowIndex * 200; // Increase delay per row
    ...
    productCard.innerHTML = `
      <div class="product-card" style="border:5px solid black;" data-aos="fade-up" data-aos-delay="${aosDelay}" tabindex="0">
      ...
      product["product-title"]
          ...
          <img src="${
            product["product-photo2"]
          }" width="312" height="350" class="image-contain" style="display: none;">
          ${saleAmount ? `<div class="card-badge">-${saleAmount}%</div>` : ""}
          ${bestSellerHTML}
          <ul class="card-action-list">
            <li class="card-action-item">
              <button class="card-action-btn add-to-cart-btn" data-product-id="${key}">
            ...
            <li class="card-action-item" onclick="addfavouriteproduct('${key}')">
              ...
              <div class="card-action-tooltip">Add to Favourite</div>
            </li>
          </ul>
        </figure>
        <div class="card-content">
          ...
          ${saleAmount ? `<del class="pre-sale">${originalPrice}</del>` : ""}
          <p class="card-price">${salePrice} EGP</p>
        </div>
      </div>`;
  ...
  AOS.refresh(); // Ensure AOS refreshes after rendering new elements
}
